...
public class CLDriver {
    ...
    private static String getJson(String inDatum, String outDatum, double[] llh, double radius, double fparm, String spcZone, String utmZone,
            String inVertDatum, String outVertDatum, double orthoHt, String heightUnits) {
        ...
        try {
            ...
            // reset height to input units
                    //reset ellipsoid height to input units
        c.setHeightUnits(heightUnits == null ? "N/A" : heightUnits);
        if (heightUnits != null && (!heightUnits.equals("m"))) {
            String[] rhgt = Nutil.resetHgt(c.getSrcEht(), c.getDestEht(), c.getSigEht(), heightUnits);
            c.setSrcEht(rhgt[0]);
            c.setDestEht(rhgt[1]);
            c.setSigEht(rhgt[2]);
            rhgt = Nutil.resetHgt(c.getSrcOrthoht(), c.getDestOrthoht(), c.getSigOrthoht(), heightUnits);
            c.setSrcOrthoht(rhgt[0]);
            c.setDestOrthoht(rhgt[1]);
            c.setSigOrthoht(rhgt[2]);
        }
    ...
    private static String toString(CoordSet c) {
        ...
        s.append(q).append("delaLat").append(q).append(":").append(q).append(c.getDeltaLat()).append(q).append(",\n");
        s.append(q).append("sigLat").append(q).append(":").append(q).append(c.getSigLat()).append(q).append(",\n");
        s.append(q).append("sigLat_m").append(q).append(":").append(q).append(c.getSigLat_m()).append(q).append(",\n");
        ...
        s.append(q).append("deltaLon").append(q).append(":").append(q).append(c.getDeltaLon()).append(q).append(",\n");
        s.append(q).append("sigLon").append(q).append(":").append(q).append(c.getSigLon()).append(q).append(",\n");
        s.append(q).append("sigLon_m").append(q).append(":").append(q).append(c.getSigLon_m()).append(q).append(",\n");
        s.append(q).append("heightUnits").append(q).append(":").append(q).append(c.getHeightUnits()).append(q).append(",\n");
...
//        s.append(q).append("spcEasting_usft").append(q).append(":").append(q).append(c.getSpcEasting_usft()).append(q).append(",\n");
    ...
    private static void convert(String[] inData) {
        ...
        String heightStr = "N/A";
        double orthoHt = Double.NaN;
        String northing = "";
        String easting = "";
        // set default height units to meters
        ...
        switch (convType) {
            ...
            case "llH":
                ...
                if (Character.isLetter(inData[4].charAt(0))) {
                    ...
                    if (inData.length > 8 && convType.equals("llh")) {
                        heightUnits = inData[8];
                    }
            ...
            case "spcH":
                ...
                if (convType.equals("spch")) {
                    ...
                    if (inData.length > 9) {
                        if (Character.isLetter(inData[9].charAt(0))) {
                            heightUnits = inData[9];
                        } else {
                            destZone = inData[9];
                    ...
                    if (inData.length > 11) {
                        if (Character.isLetter(inData[11].charAt(0))) {
                            heightUnits = inData[11];
                        } else {
                            destZone = inData[11];
                            if (!Nutil.isValid("spczone", destZone)) {
                                System.err.println("Invalid Destination SPC zone");
            ...
            case "utmH":
                ...
                if (convType.equals("utmh")) {
                    ...
                    if (Character.isLetter(inData[5].charAt(0))) {
                        ...
                        heightStr = inData[7];
                        if (inData.length > 8) {
                            if (Character.isLetter(inData[8].charAt(0))) {
                                heightUnits = inData[8];
                            } else {
                                destZone = inData[8];
                                if (!Nutil.isValid("utmzone", destZone)) {
                ...
                if (convType.equals("utmH")) {
                    ...
                    // UTM is being converted between zones
                    if (inData.length > 11) {
                        if (Character.isLetter(inData[11].charAt(0))) {
                            heightUnits = inData[11];
                        } else {
            ...
            case "usngH":
                ...
                if (convType.equals("usngh")) { //ellipsoid height
        ...
        if (!destZone.equals("auto")) {
            if (destZone.length() > 2) {
                ...
                utmZone = destZone;
                           ...
                           llh[2] = Nutil.toMeters(llh[2], heightUnits);
                           orthoHt = Nutil.toMeters(orthoHt, heightUnits);

        System.out.println(getJson(inDatum, outDatum, llh, radius, fparm, spcZone, utmZone,
                inVertDatum, outVertDatum, orthoHt,heightUnits));

    }

    public static void main(String[] args) {
              String[] inData = System.getProperty("parms", "").split(",");
 //       String[] inData = "xyz,-217687.279,-5069012.406,3852223.048,NAD83(2011),NAD83(NSRS2007),2402,auto".split(",");
        if (!(inData.length > 1)) {
